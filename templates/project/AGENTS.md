...
evaluation specs, and bindings directly. On macOS (no user namespaces), set
`network: allow` on dev scripts and `permitUnisolated: true` in the provider
config; `deny` stages fail closed.
